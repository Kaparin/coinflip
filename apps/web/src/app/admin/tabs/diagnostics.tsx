...
export function DiagnosticsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
          Здоровье хранилища (AXM Vault)
        </h2>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <StatCard label="Юзеры с балансом" value={d?.vault.totalUsers ?? '...'} />
          <StatCard label="Всего AXM доступно" value={d ? formatLaunch(d.vault.totalAvailable) : '...'} />
          <StatCard label="Всего AXM locked" value={d ? formatLaunch(d.vault.totalLocked) : '...'} />
          <StatCard label="С locked балансом" value={d?.vault.usersWithLocked ?? '...'} />
      ...
      {d?.stuckLockedFunds && d.stuckLockedFunds.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-bold uppercase tracking-wider text-[var(--color-danger)]">
            Застрявшие AXM средства ({d.stuckLockedFunds.length})
          </h2>
          <p className="text-xs text-[var(--color-text-secondary)]">
            Пользователи с заблокированным AXM балансом, но без активных ставок. Средства фактически заморожены.
          </p>
          <TableWrapper>
