    ...
    switch (event.type) {
      ...
      case 'bet_revealed': {
        ...
        if (duel) {
          ...
          // resolving(0) → avatar-merge(+500ms) → winner-reveal(+4200ms) → fade-out(+12200ms) → remove
          // CSS coin toss is 3.5s, spin is 3s — winner reveal after both complete
          const t1 = setTimeout(() => updatePhase(betId, 'avatar-merge'), 500);
