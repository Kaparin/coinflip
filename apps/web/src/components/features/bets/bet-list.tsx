  ...
  const acceptMutation = useAcceptBet({
    mutation: {
      onSuccess: (response: any, variables) => {
        ...
        // NOTE: Do NOT remove from recentlyAcceptedIds here. The bet might still
        // be 'open' in the DB until the relay processes it. If a poll refetch runs
        // before the WS bet_accepting event, the card would reappear. Keeping
        // the ID in recentlyAcceptedIds prevents this race condition.
        // IDs are session-scoped and small, so no cleanup needed.
              ...
              <FlipCard
                ...
                front={
                  <BetCard
                    ...
                    onAccept={isMyBet || !isConnected || recentlyAcceptedIds.has(betId) ? undefined : handleAcceptClick}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between rounded-lg bg-emerald-500/5 border border-emerald-500/10 p-3">
                ...
                <span className="flex items-center gap-1.5 text-lg font-bold tabular-nums text-emerald-400">
                  +{winAmount.toLocaleString('en-US', { maximumFractionDigits: 2 })} <GameTokenIcon size={18} />
                </span>
              </div>
