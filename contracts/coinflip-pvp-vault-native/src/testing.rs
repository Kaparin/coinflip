...
pub mod helpers {
    ...
    /// Deposit native tokens for a user (sends funds with Deposit message)
    pub fn deposit(
        ...
        let info = mock_info(user, &coins(amount, DENOM));
        execute(deps.as_mut(), env.clone(), info, ExecuteMsg::Deposit {})
    }
